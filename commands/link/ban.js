...
const { getOwnedRealms, banPlayer } = require("../../src/realm.js");
...
module.exports = {
  name: "ban",
  description: "Bans a member from a selected owned realm.",
  cooldown: 5000,
  ...
  execute: async (interaction, args, dbUser) => {
    ...
    embed.description = "Choose an realm to blacklist the player from.";
...
async function selected(interaction) {
  ...
  const data = await banPlayer(interaction.user.id, realmID, xuid);

  if (data === 200) embed.description = `Blacklisted **${savedArgs.username}** from your realm.`;
  if (data != 200) embed.description = `Failed to blacklist **${savedArgs.username}** from your realm.`;
  if (data === 502 || data === 504) embed.description = `Backend Realms API is down.`;

  interaction.message.edit({ embed, components: [], flags: 64 });
}